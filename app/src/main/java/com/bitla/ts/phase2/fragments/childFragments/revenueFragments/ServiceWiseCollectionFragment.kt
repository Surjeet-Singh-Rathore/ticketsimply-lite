...
import com.bitla.ts.databinding.FragmentServiceWiseBinding
...
class ServiceWiseCollectionFragment : BaseFragment(), OnItemClickListener {

    companion object {
        val TAG = ServiceWiseCollectionFragment::class.java.simpleName
    }

    private lateinit var binding: FragmentServiceWiseBinding
    ...
    private var sortBy = "htol_netamt"
    ...
    override fun onCreateView(
        ...
        binding = FragmentServiceWiseBinding.inflate(inflater, container, false)
        ...
        binding.apply {
            ...
            /*btnChangeChart.setOnClickListener {
                DialogUtils.dialogChartPopup(
                    requireContext(),
                    this@ServiceWiseCollectionFragment
                )
...
//            binding.barChartCommon.barChart.setVisibleXRangeMaximum(1f)
        ...
        dashboardViewModel.revenueDetailsResponseViewModel.observe(viewLifecycleOwner) {
            ...
            if (it.code == 200) {
                try {
                    ...
                    if (occupancyDetailsResponse.result.serviceWiseCollection.isNullOrEmpty()
                            .not()
                    ) {
                        occupancyDetailsResponse.result.serviceWiseCollection?.forEachIndexed { i, element ->

                            if (element.revenue.isEmpty()) {
                                var newRevenue =
                                    element.revenue.toString().replace(privilegeResponse?.currency ?: "", "").toDouble()
                                ...
                                occupancyReportList.add(
                                    ReportValue(
                                        element.service.substring(
                                            SUBSTRING_START,
                                            min(SUBSTRING_END, element.service.length)
                        ...
                        setDefaultChart()
                        binding.dashboardDetailsServiceWiseCard.visible()
                        binding.noData.root.gone()
                    } else {
                        binding.dashboardDetailsServiceWiseCard.gone()
