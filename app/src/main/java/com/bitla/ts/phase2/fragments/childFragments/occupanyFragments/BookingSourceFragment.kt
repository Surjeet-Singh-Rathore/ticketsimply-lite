...
import com.bitla.ts.databinding.FragmentBookingSourceBinding
...
class BookingSourceFragment : BaseFragment(), OnItemClickListener {

    companion object {
        val TAG = BookingSourceFragment::class.java.simpleName
    }

    private var privileges: PrivilegeResponseModel? = null
    private lateinit var binding: FragmentBookingSourceBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentBookingSourceBinding.inflate(inflater, container, false)
                    ...
                    object : DialogSingleButtonListener {
                        override fun onSingleButtonClick(str: String) {
                            ...
                            firebaseLogEvent(
                                ...
                                loginModelPref.role,
                                SORT_BTN,
                                SortBtn.BOOKING_SOURCE_WISE_SORT
                            )
                        }
                    }
                )
...
/*
        if(requireActivity().intent.hasExtra(getString(R.string.dashboardGraphToDate))){
            toDate = requireActivity().intent.getStringExtra(getString(R.string.dashboardGraphToDate)) ?: fromDate
            requireActivity().toast(toDate)
        }*/
    ...
    private fun callOccupancyDetailsApi() {
        ...
        if (requireActivity().isNetworkAvailable()) {
            ...
            binding.apply {
                ...
                stopShimmerEffect()
                dashboardDetailsBranchSourceLayout.gone()
                noData.root.visible()
        ...
        dashboardViewModel.occupancyDetailsResponseViewModel.observe(viewLifecycleOwner) {
            try {
                ...
                if (it.code == 200) {
                    ...
                    occupancyDetailsResponse.result.occupancyByBookingSource?.forEachIndexed { i, _ ->
//                    if (DASHBOARD_CHART_LIMIT > occupancyDayWiseList.size) { }
                        occupancyReportList.add(
                            ReportValue(
                                occupancyDetailsResponse.result.occupancyByBookingSource[i].bookingSource,
                                occupancyDetailsResponse.result.occupancyByBookingSource[i].occupancy.toDouble()
                                    .toString(),
                                getDashboardChartColor(
                                    requireContext(),
                                    occupancyDetailsResponse.result.occupancyByBookingSource[i].occupancy.toFloat()
                                        .roundToInt()
                                ),
                                occupancyDetailsResponse.result.occupancyByBookingSource[i].bookingSource,
                                "${occupancyDetailsResponse.result.occupancyByBookingSource[i].occupancy.toFloat()}%"
                        ...
                        occupancyDayWiseList.add(
                            OccupancyAllTabsDetailsModel(
                                occupancyDetailsResponse.result.occupancyByBookingSource[i].bookingSource,
                                occupancyDetailsResponse.result.occupancyByBookingSource[i].occupancy.plus(
                                    "%"
                                ),
                            )
                        )
                    ...
                    binding.dashboardDetailsBranchSourceLayout.visible()
                    binding.dashboardDetailsBranchSourceCard.visible()
                    binding.noData.root.gone()

                } else {
                    if (it?.message != null) {
                        binding.dashboardDetailsBranchSourceLayout.visible()
                        binding.dashboardDetailsBranchSourceCard.gone()
...
//                initAndSetupPieChart()
    ...
    private fun stopShimmerEffect() {
        binding.apply {
            ...
            dashboardDetailsBranchSourceLayout.visible()
        ...
        pickUpChartViewModel.allotedDetailResponse.observe(viewLifecycleOwner) { it ->
            if (it?.services != null) {
                ...
                binding.allServicesAndDateFilterContainer.tvAllService.text =
                    "${getString(R.string.all_cities)} (${it.services.size})"
