...
import com.bitla.ts.databinding.FragmentBranchAccountsSummaryBinding
...
import com.bitla.ts.presentation.viewModel.BranchWiseRevenueViewModel
...
import com.bitla.ts.utils.common.getDashboardChartColor
import com.bitla.ts.utils.common.getDateYMD
import com.bitla.ts.utils.common.getTodayDate
import com.bitla.ts.utils.common.setCurrencyBranchFormatBarChart
import com.bitla.ts.utils.common.setCurrencyLineChart
import com.bitla.ts.utils.common.setPieChart
import com.bitla.ts.utils.common.shareView
import com.bitla.ts.utils.constants.CHANGE_GRAPH
import com.bitla.ts.utils.constants.CUSTOM_ELLIPSIS
import com.bitla.ts.utils.constants.ChangeGraph
...
import com.bitla.ts.utils.constants.SUBSTRING_END
import com.bitla.ts.utils.constants.SUBSTRING_START
import com.bitla.ts.utils.constants.SortBtn
import com.bitla.ts.utils.dialog.DialogUtils
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_MODEL_BRANCH_WISE_REVENUE
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_REVENUE_DESTINATION_CITY_NAME
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_REVENUE_FROM_DATE
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_REVENUE_ORIGIN_CITY_NAME
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_REVENUE_SERVICE_ID
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_REVENUE_SERVICE_NAME
import com.bitla.ts.utils.sharedPref.BRANCH_WISE_REVENUE_TO_DATE
...
class BranchWiseRevenueFragment : BaseFragment(), OnItemClickListener {

    companion object {
        val TAG = BranchWiseRevenueFragment::class.java.simpleName
    }

    private lateinit var binding: FragmentBranchAccountsSummaryBinding
    private val branchWiseRevenueViewModel by viewModel<BranchWiseRevenueViewModel<Any>>()
    ...
    private var toDate = ""
    private lateinit var bindingDialog: DashboardBranchWiseRevenuePopupBinding
    ...
    private var revenueDetailsResponse:RevenueDetailsResponse? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentBranchAccountsSummaryBinding.inflate(inflater, container, false)
...
//                            callRevenueDetailsApi()
        ...
        firebaseLogEvent(
            ...
            loginModelPref.role,
            SORT_BTN,
            SortBtn.BRANCH_WISE_REVENUE_SORT
        )
        
        binding.searchET.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable) {
                if (::occupancyAllTabsDetailsAdapter.isInitialized) {
                    occupancyAllTabsDetailsAdapter.filter.filter(s.toString())
                }
            }
    ...
    private fun setBranchWisePopUpObserver(){
        branchWiseRevenueViewModel.branchWiseRevenueDetails.observe(viewLifecycleOwner) {
            try {

            if (it.code == 200) {
                DialogUtils.dialogBranchWiseRevenue(requireContext(), onItemClickListener = this,it?.result?.details ?: listOf(), branchName = selectedBranchName ?: "")
    ...
    private fun setOccupancyDayWiseListAdapter() {
        ...
        occupancyAllTabsDetailsAdapter =
            OccupancyAllTabsDetailsAdapter(
               ...
               selectedBranchName = revenueDetailsResponse?.result?.branchAccountsSummary?.get(it)?.branch  ?: ""

//                callBranchWiseRevenueApi(revenueDetailsResponse?.result?.branchAccountsSummary?.get(it)?.branchId ?: "")
            }
        binding.rvOccupancyDayWise.adapter = occupancyAllTabsDetailsAdapter
    }
    private fun callBranchWiseRevenueApi(branchId:String){
        if (requireActivity().isNetworkAvailable()) {
            branchWiseRevenueViewModel.branchWiseRevenueDetailsApi(
            ...
            if (it.code == 200) {
                try {
                    ...
                    val occupancyDetailsResponse = it
                       revenueDetailsResponse = it
                    if (occupancyDetailsResponse.result.branchAccountsSummary?.isNotEmpty() == true) {
                        occupancyDetailsResponse.result.branchAccountsSummary.forEachIndexed { i, _ ->
//                        if (DASHBOARD_CHART_LIMIT > occupancyReportList.size) { }

                            val newRevenue =
                                occupancyDetailsResponse.result.branchAccountsSummary[i].revenue
                            ...
                            if (occupancyDetailsResponse.result.branchAccountsSummary[i].revenue.toDouble() < 0) {
                                isNegativeValue = true
                            }
                            occupancyReportList.add(
                                ReportValue(
                                    occupancyDetailsResponse.result.branchAccountsSummary[i].branch.substring(
                                        SUBSTRING_START, min(
                                            SUBSTRING_END,
                                            occupancyDetailsResponse.result.branchAccountsSummary[i].branch.length
                                        )
                                    ).plus(CUSTOM_ELLIPSIS),
                                    occupancyDetailsResponse.result.branchAccountsSummary[i].revenue.toDouble()
                                        .toString(),
                                    getDashboardChartColor(
                                        requireContext(),
                                        occupancyDetailsResponse.result.branchAccountsSummary[i].revenue.toDouble()
                                    ...
                                    occupancyDetailsResponse.result.branchAccountsSummary[i].branch,
                            ...
                            occupancyDayWiseList.add(
                                OccupancyAllTabsDetailsModel(
                                    occupancyDetailsResponse.result.branchAccountsSummary[i].branch,
                                    "${privilegeResponse?.currency ?: ""}${occupancyDetailsResponse.result.branchAccountsSummary[i].revenue}",
                                    grossRevenue = "${privilegeResponse?.currency ?: ""}${occupancyDetailsResponse.result.branchAccountsSummary[i].grossRevenue ?: "0.0"}",
                                    netRevenue = "${privilegeResponse?.currency ?: ""}${occupancyDetailsResponse.result.branchAccountsSummary[i].revenue}"
                                )
                            )
                        }
                    ...
                    Timber.d("Error in BranchRevenueFragment OccupancyDetails Observer ${e.message}")
                }
            } else {
                if (it?.message != null) {
                    stopShimmerEffect()
