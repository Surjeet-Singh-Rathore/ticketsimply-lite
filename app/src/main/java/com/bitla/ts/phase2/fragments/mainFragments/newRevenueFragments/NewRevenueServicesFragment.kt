...
import com.bitla.ts.presentation.view.activity.SearchActiveInactiveServiceActivity
...
class NewRevenueServicesFragment : BaseFragment(), com.bitla.ts.data.listener.OnItemClickListener {
    ...
    override fun onCreateView(
        ...
        binding.allServicesAndDateFilterContainer.tvAllService.setOnClickListener {
            val intent = Intent(requireContext(), SearchActiveInactiveServiceActivity::class.java)
        ...
        binding.allServicesAndDateFilterContainer.tvAllService.setOnClickListener {
            ...
            val intent: Intent = Intent(
                requireContext(), SearchServiceActivity::class.java
            )
            intent.putExtra(
    ...
    private fun setAdapter(revenueRouteList: ArrayList<RevenueRouteDetails>?) {
        if (revenueRouteList?.size!! > 0) {
            adapter = RevenueServicesAdapter(
                requireContext(), revenueRouteList, privilegeResponse, this
            )
    ...
    private fun getRevenueDataObserver() {
        dashboardRevenueViewModel.revenueData.observe(requireActivity(), Observer {
            ...
            } else requireActivity().toast(getString(R.string.server_error))
        })
    }
...
//        }
    ...
    private fun setAllServices() {
        ...
        allottedServicesResponseModel?.services?.forEach {
            if (it.isChecked) totalSelectedServices++
        }
    ...
    private fun revenueDateSetText(
        fromDate: String, toDate: String?, inputDateFormat: String
    ) {
        var text = ""
        if (toDate != null) {
            if (toDate != "") {
                text = "${
                    getCurrentFormattedDate(
                        fromDate, inputDateFormat, DATE_FORMAT_MMM_DD
                    )
