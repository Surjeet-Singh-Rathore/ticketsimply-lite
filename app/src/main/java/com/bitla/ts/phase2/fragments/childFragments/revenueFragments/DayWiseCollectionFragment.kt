...
import com.bitla.ts.databinding.FragmentDayWiseCollectionBinding
...
import com.bitla.ts.phase2.dashboard_pojo.dashboardAllModels.revenue_model.request.RevenueDetailsRequest
...
class DayWiseCollectionFragment : BaseFragment(), OnItemClickListener {

    companion object {
        val TAG = DayWiseCollectionFragment::class.java.simpleName
    }

    private lateinit var binding: FragmentDayWiseCollectionBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentDayWiseCollectionBinding.inflate(inflater, container, false)
        val view: View = binding.root

        PreferenceUtils.removeKey(DAY_WISE_COLLECTION_REVENUE_TO_DATE)
        PreferenceUtils.removeKey(DAY_WISE_COLLECTION_REVENUE_FROM_DATE)
        //PreferenceUtils.removeKey(DAY_WISE_COLLECTION_REVENUE_ORIGIN_CITY_NAME)
        PreferenceUtils.removeKey(DAY_WISE_COLLECTION_REVENUE_DESTINATION_CITY_NAME)
        ...
        binding.apply {
            ...
            tvChartStatusTitle.text = "(${privilegeResponse?.currency}) Total Revenue"
                    ...
                    object : DialogSingleButtonListener {
                        override fun onSingleButtonClick(str: String) {
                            ...
                            firebaseLogEvent(
                                ...
                                loginModelPref.role,
                                SORT_BTN,
                                SortBtn.DAY_WISE_COLLECTION_REVENUE_SORT
                            )
            ...
            /*btnChangeChart.setOnClickListener {
                DialogUtils.dialogChartPopup(
                    requireContext(),
                    this@DayWiseCollectionFragment
                )
                legendValues.clear()
                entries.clear()
                colors.clear()
...
//                            callRevenueDetailsApi()
                            ...
                            dashboardServiceFilterConf.toTitle = getString(R.string.all_services_title_case)
...
/*            if(getDaysDifference(fromDate, toDate, DATE_FORMAT_Y_M_D) < 2 && !isLocalFilter) {
            ...
            binding.apply {
                ...
                stopShimmerEffect()
                dashboardDetailsDayWiseCollectionLayout.gone()
                noData.root.visible()
        ...
        dashboardViewModel.revenueDetailsResponseViewModel.observe(viewLifecycleOwner) {
            ...
            if (it != null) {
                if (it.code == 200) {
                    ...
                    try {
                        occupancyDetailsResponse.result.dayWiseCollection?.forEachIndexed { i, element ->
                            if (DASHBOARD_CHART_LIMIT > occupancyDayWiseList.size) {
                                var newRevenue = element.revenue.toString().replace(privilegeResponse?.currency ?: "", "").toDouble()
                                    .convert(privilegeResponse?.currencyFormat ?: requireActivity().getString(R.string.indian_currency_format))
                                if (element.revenue.toDouble() < 0) {
                                    isNegativeValue = true
                                }
                                occupancyReportList.add(
                                    ReportValue(
                                        getDateMMMDD(element.date).toString(),
                                        element.revenue.toDouble().toString(),
                                        getDashboardChartColor(
                                            requireContext(),
                                            element.revenue.toDouble().toInt()
                                        ),
                                        getDateMMMDD(element.date).toString(),
                            ...
                            occupancyDayWiseList.add(
                                OccupancyAllTabsDetailsModel(
                                    getDateMMMDD(element.date).toString(),
                                    "${privilegeResponse?.currency ?: ""}${element.revenue}",
                                    grossRevenue = "${privilegeResponse?.currency ?: ""}${element.grossRevenue ?: ""}",
                                    netRevenue = "${privilegeResponse?.currency ?: ""}${element.revenue}"
                                )
                            )
                        }
                    ...
                    binding.dashboardDetailsDayWiseCollectionLayout.visible()
                    binding.dashboardDetailsDayWiseCollectionCard.visible()
                    binding.noData.root.gone()
                } else {
                    if (it.message != null) {
                        stopShimmerEffect()
                        binding.dashboardDetailsDayWiseCollectionLayout.visible()
                        binding.dashboardDetailsDayWiseCollectionCard.gone()
...
//                initAndSetupPieChart()
    ...
    private fun stopShimmerEffect() {
        binding.apply {
            ...
            dashboardDetailsDayWiseCollectionLayout.visible()
