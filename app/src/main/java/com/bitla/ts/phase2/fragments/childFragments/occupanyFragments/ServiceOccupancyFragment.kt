...
import com.bitla.ts.databinding.FragmentServiceOccupancyBinding
...
import com.bitla.ts.presentation.viewModel.ServiceOccupancyViewModel
...
import com.bitla.ts.utils.common.setDecimalFormatBarChart
import com.bitla.ts.utils.common.setLineChart
...
class ServiceOccupancyFragment : BaseFragment(), OnItemClickListener {
    
    companion object {
        val TAG = ServiceOccupancyFragment::class.java.simpleName
    }

    private var privileges: PrivilegeResponseModel? = null
    private val serviceWiseOccupancyViewModel by viewModel<ServiceOccupancyViewModel<Any>>()
    private var dialogSubHeader = ""
    private lateinit var binding: FragmentServiceOccupancyBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentServiceOccupancyBinding.inflate(inflater, container, false)
    ...
    private fun setServiceWisePopUpObserver() {
        serviceWiseOccupancyViewModel.serviceWiseOccupancyDetails.observe(viewLifecycleOwner) {
            try {
                if (it != null) {
                    if (it.code == 200) {
                        DialogUtils.dialogServiceWiseOccupancy(
                            requireContext(),
                            onItemClickListener = this,
                            it?.result?.details ?: listOf(),
                            serviceId = dialogSubHeader ?: ""
                        )
...
//        }
    ...
    private fun callServiceWiseOccupancyApi(routeId: String) {
        if (requireActivity().isNetworkAvailable()) {
            serviceWiseOccupancyViewModel.serviceOccupancyDetailsApi(
    ...
    private fun setOccupancyDayWiseListAdapter() {
        ...
        occupancyAllTabsDetailsAdapter =
            OccupancyAllTabsDetailsAdapter(requireActivity(),privileges, occupancyDayWiseList) {
                dialogSubHeader =
                    occupancyDayWiseList.get(it).title + " - " + occupancyDayWiseList.get(it).value
                
                callServiceWiseOccupancyApi(
                    occupancyDetailsResponse?.result?.serviceOccupancy?.get(
                        it
                    )?.routeId ?: ""
                )
            }
    ...
    private fun callOccupancyDetailsApi() {
        ...
        if (requireActivity().isNetworkAvailable()) {
            ...
            binding.apply {
                ...
                stopShimmerEffect()
                dashboardDetailsServiceWiseLayout.gone()
                noData.root.visible()
        ...
        dashboardViewModel.occupancyDetailsResponseViewModel.observe(viewLifecycleOwner) {
            ...
            if (it.code == 200) {
                try {
                    ...
                    if (occupancyDetailsResponse.result.serviceOccupancy?.isNotEmpty() == true) {
                        occupancyDetailsResponse.result.serviceOccupancy?.forEachIndexed { i, element ->
                            ...
                            occupancyReportList.add(
                                ReportValue(
                                    ...
                                    element.occupancy.toDouble().toString(),
                                    getDashboardChartColor(
                                        requireContext(),
                                        element.occupancy.toDouble().toInt()
                                    ),
                                    element.service,
                                    "${element.occupancy.toFloat()}%"
                                )
                            )
                        ...
                        stopShimmerEffect()
                        binding.seviceWiseOccupancyCard.visible()
                        binding.noData.root.gone()
                    } else {
                        stopShimmerEffect()
                        binding.seviceWiseOccupancyCard.gone()
                    ...
                    Timber.d("Error in ServiceOccupancyFragment OccupancyDetails Observer ${e.message}")
                }
...
//                initAndSetupPieChart()
    ...
    private fun stopShimmerEffect() {
        binding.apply {
            ...
            dashboardDetailsServiceWiseLayout.visible()
    ...
    private fun setAllServices() {
        ...
        if (serviceId == "-1" && totalSelectedServices == totalOverallServices) {
            ...
            selectedServiceName = "${getString(R.string.all_cities)} (${totalOverallServices})"
