...
import com.bitla.ts.databinding.FragmentServiceTaxCollectionBinding
...
import com.bitla.ts.utils.constants.SortBtn.GST_COLLECTION_REVENUE_SORT
...
class GSTCollectionFragment : BaseFragment(), OnItemClickListener {

    companion object {
        val TAG = GSTCollectionFragment::class.java.simpleName
    }

    private var privileges: PrivilegeResponseModel? = null
    private lateinit var binding: FragmentServiceTaxCollectionBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentServiceTaxCollectionBinding.inflate(inflater, container, false)
        ...
        binding.apply {
            ...
            sortLayout.btnSort.setOnClickListener {
                DialogUtils.dialogSortBy(
                    ...
                    sortBy = sortBy,
                    newLowToHighLabel = requireActivity().getString(R.string.gst_collection_revenue_sort_ltoh),
                    newHighToLowLabel = requireActivity().getString(R.string.gst_collection_revenue_sort_htol),
...
//        }
    ...
    private fun setOccupancyDayWiseListAdapter() {
        ...
        occupancyAllTabsDetailsAdapter =
            OccupancyAllTabsDetailsAdapter(requireActivity(),privileges, occupancyDayWiseList, false, true)
...
/*            if(getDaysDifference(fromDate, toDate, DATE_FORMAT_Y_M_D) < 2 && !isLocalFilter) {
            ...
            binding.apply {
                ...
                stopShimmerEffect()
                dashboardDetailsGstTaxLayout.gone()
                noData.root.visible()
        ...
        dashboardViewModel.revenueDetailsResponseViewModel.observe(viewLifecycleOwner) {
            ...
            if (it.code == 200) {
                try {
                    ...
                    if (occupancyDetailsResponse.result.serviceTaxCollection.isNullOrEmpty()
                            .not()
                    ) {
                        occupancyDetailsResponse.result.serviceTaxCollection?.forEachIndexed { i, element ->
//                        if (DASHBOARD_CHART_LIMIT > occupancyDayWiseList.size) { }
                            var newRevenue = element.revenue.toString().replace("₹", "").toDouble()
                                .convert(context!!.getString(R.string.indian_currency_format))
                            ...
                            occupancyDayWiseList.add(
                                OccupancyAllTabsDetailsModel(element.service, "₹${element.revenue}")
                            )
                        }

                        setOccupancyDayWiseListAdapter()
                        setDefaultChart()
                        binding.dashboardDetailsGstTaxCard.visible()
                        binding.noData.root.gone()
                    } else {
                        binding.dashboardDetailsGstTaxCard.gone()
                    ...
                    Timber.d("Error in GSTCollectionFragment OccupancyDetails Observer ${e.message}")
                }
            } else {
                if (it?.message != null) {
                    stopShimmerEffect()
...
//                initAndSetupPieChart()
    ...
    private fun stopShimmerEffect() {
        binding.apply {
            ...
            dashboardDetailsGstTaxLayout.visible()
