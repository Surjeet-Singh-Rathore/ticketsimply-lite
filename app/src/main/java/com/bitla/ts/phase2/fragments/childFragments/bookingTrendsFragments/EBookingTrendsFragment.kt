...
import com.bitla.ts.databinding.FragmentBestDaysBinding
...
class EBookingTrendsFragment : BaseFragment(), OnItemClickListener {

    companion object {
        val TAG = EBookingTrendsFragment::class.java.simpleName
    }

    private lateinit var binding: FragmentBestDaysBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentBestDaysBinding.inflate(inflater, container, false)
                    ...
                    object : DialogSingleButtonListener {
                        override fun onSingleButtonClick(str: String) {
                            ...
                            firebaseLogEvent(
                                ...
                                loginModelPref.role,
                                SORT_BTN,
                                SortBtn.E_BOOKING_TRENDS_BOOKINGS_TRENDS_SORT
                            )
                        }
                    }
                )
                    ...
                    object : DialogSingleButtonListener {
                        override fun onSingleButtonClick(str: String) {
                            ...
                            firebaseLogEvent(
                                ...
                                loginModelPref.role,
                                SORT_BTN,
                                "E-Booking Trends Booking Trends sort"
                            )
                        }
                    }
                )
...
//        }
    ...
    private fun setOccupancyDayWiseListAdapter() {
        ...
        occupancyAllTabsDetailsAdapter =
            OccupancyAllTabsDetailsAdapter(requireActivity(),privilegeResponse, occupancyDayWiseList)
    ...
    private fun getPref() {
        ...
        fromDate = PreferenceUtils.getDashboardFromPastDate()
...
/*            if(getDaysDifference(fromDate, toDate, DATE_FORMAT_Y_M_D) < 2 && !isLocalFilter) {
            ...
            binding.apply {
                ...
                stopShimmerEffect()
                dashboardDetailsEBookingTrendsLayout.gone()
                noData.root.visible()
            ...
            if (it != null) {
                if (it.code == 200) {
                    try {
                        occupancyReportList.clear()
                        occupancyDayWiseList.clear()
                        val bookingTrendsResponse = it

                        if (bookingTrendsResponse.result.eTicketsPerformance?.isNotEmpty() == true) {

                            bookingTrendsResponse.result.eTicketsPerformance.forEachIndexed { i, _ ->
//                        if (DASHBOARD_CHART_LIMIT > occupancyDayWiseList.size) { }
                                occupancyReportList.add(
                                    ReportValue(
                                        getDateMMMDD(bookingTrendsResponse.result.eTicketsPerformance[i].eTicket).toString(),
                                        bookingTrendsResponse.result.eTicketsPerformance[i].performance.toString(),
                                        getDashboardChartColor(
                                            requireContext(),
                                            bookingTrendsResponse.result.eTicketsPerformance[i].performance.toFloat()
                                                .roundToInt()
                                        ),
                                        getDateMMMDD(bookingTrendsResponse.result.eTicketsPerformance[i].eTicket).toString(),
                                        "${bookingTrendsResponse.result.eTicketsPerformance[i].performance}%"
                                    )
                                )
...
/*
                            if (occupancyReportList.isNotEmpty()&& occupancyReportList.size==2){
                                if (getDateMMMDD(toDate) != occupancyReportList[1].name
                                    ...
                                    occupancyDayWiseList.add(
                                        OccupancyAllTabsDetailsModel (getDateMMMDD(toDate).toString(), "0.0%"))
                                }
                                    ...
                                    occupancyDayWiseList.add(OccupancyAllTabsDetailsModel(getDateMMMDD(toDate).toString(), "0%"))
                                    occupancyDayWiseList.add(OccupancyAllTabsDetailsModel(getDateMMMDD(toDate).toString(), "0%"))
                                    occupancyDayWiseList.add(OccupancyAllTabsDetailsModel(getDateMMMDD(toDate).toString(), "0%"))
                                }
                            }
*/
                            ...
                            /*else if (sortByCount>0 && sortBy=="htol"){
                                occupancyReportList.sortByDescending { it.name }
                                occupancyDayWiseList.sortByDescending { it.title }
                            }*/

                            binding.dashboardDetailsEBookingTrendsLayout.visible()
                            binding.dashboardDetailsEBookingTrendsCard.visible()
                            ...
                            binding.dashboardDetailsEBookingTrendsLayout.visible()
                            binding.dashboardDetailsEBookingTrendsCard.gone()
                        ...
                        Timber.d("Error in EBookingTrendsFragment OccupancyDetails Observer ${e.message}")
                    }
                } else {
                    if (it.message != null) {
                        stopShimmerEffect()
...
//                initAndSetupPieChart()
    ...
    private fun stopShimmerEffect() {
        binding.apply {
            ...
            dashboardDetailsEBookingTrendsLayout.visible()
