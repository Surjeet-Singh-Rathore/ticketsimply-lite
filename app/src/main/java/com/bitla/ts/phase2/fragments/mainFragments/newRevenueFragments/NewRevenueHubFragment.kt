...
class NewRevenueHubFragment : BaseFragment(), com.bitla.ts.data.listener.OnItemClickListener {
    ...
    override fun onCreateView(
        ...
        binding.allServicesAndDateFilterContainer.tvAllService.setOnClickListener {
            ...
            intent.putExtra("isFromHubRevenue", true)
...
//        }
    ...
    private fun setAllServices() {
        ...
        if (totalSelectedServices > 0 && totalSelectedServices < totalOverallServices
            ...
            serviceId = "-1"
            selectedServiceName =
                "${getString(R.string.all_hubs)} ($totalOverallServices)"
        ...
        if (serviceId == "-1" && totalSelectedServices == totalOverallServices) {
            ...
            selectedServiceName = "${getString(R.string.hubs)} ($totalSelectedServices)"
                ...
                if(it.code == 200) {
                    if (it.hub_list.isNotEmpty()) {
                        binding.allServicesAndDateFilterContainer.tvAllService.text =
                            "${getString(R.string.hubs)} (${it.hub_list.size})"
                    }
                } else if(it.code == 401){
                   // openUnauthorisedDialog()
                    (activity as BaseActivity).showUnauthorisedDialog()
