...
class AllPassengersFragment : BaseFragment(), View.OnClickListener, DialogButtonListener,
    OnItemClickListener, DialogSingleButtonListener {
    companion object {
        val tag: String = AllPassengersFragment::class.java.simpleName
    }
    ...
    lateinit var binding: FragmentAllPassengersBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentAllPassengersBinding.inflate(inflater, container, false)
        getPref()
        setObserver()
        initUi()
        ...
        if (smsTemplate != null && smsPassengerType != null && smsPassengerType == AllPassengersFragment.tag)
    ...
    private fun getPref() {
        ...
        if (PreferenceUtils.getObject<Result>(PREF_SELECTED_AVAILABLE_ROUTES) != null) {
            ...
            busType = result?.dep_time ?: getString(R.string.empty)
        }
    ...
    private fun setObserver() {
        allPassengersViewModel.etOnChange.observe(requireActivity(), androidx.lifecycle.Observer {
            if (!it) {
    ...
    private fun initUi() {
        val srcDest = "$source to $destination"
        val dateTime = "$travelDate at $deptTime"
        binding.apply {
            layoutNotify.tvNotifyTitle.text = getString(R.string.notifyAll)
    ...
    private fun clickListener() {
        binding.etSelectSms.setOnClickListener(this)
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            R.id.etSelectSms -> {
                ...
                PreferenceUtils.putString(PREF_SMS_PASSENGER_TYPE, AllPassengersFragment.tag)
                    ...
                    /*else if (!binding.chkSmsAll.isChecked) {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            source?.let {
                                destination?.let { it1 ->
                                    coachNumber?.let { it2 ->
                                        DialogUtils.sendSmsDialog(
                                            title = getString(/* resId = */ R.string.send_sms),
                                            message = binding.tvMessage.text.toString(),
                                            btnLeftText = getString(R.string.dont_send_sms),
                                            btnRightText = getString(R.string.send_sms),
                                            source = it,
                                            destination = it1,
                                            coachNumber = it2,
    ...
    override fun onRightButtonClick() {
        if (requireContext().isNetworkAvailable()) {
            ...
            val delay =
                Delay(time, binding.acHour.text.toString(), binding.acMinutes.text.toString())
            ...
            val reqBody = employee.let {
                smsId?.toString()?.let { it1 ->
                    custom?.let { it2 ->
                        ReqBody(
                            apiKey, bus, charts, delay, driver, it, resId.toString(), mobile,
                            it1, it2, pnrNos, is_from_middle_tier, locale = locale
                        )
                    }
                }
            }
