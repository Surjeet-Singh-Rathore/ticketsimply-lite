package com.bitla.ts.presentation.view.passenger_payment
...
class PassengerPaymentActivity : BaseActivity(), VarArgListener, DialogSingleButtonListener,
    ...
    private var pickupChargeDetailsRequest = ChargeDetails()
    private var dropoffChargeDetailsRequest = ChargeDetails()
...
//        }
        ...
        if(intent.hasExtra("pickupAddressCharge") && intent?.extras?.get("pickupAddressCharge") != null) {
            pickupChargeDetailsRequest = intent?.extras?.get("pickupAddressCharge") as ChargeDetails
        }
        if(intent.hasExtra("dropoffAddressCharge") && intent?.extras?.get("dropoffAddressCharge") != null) {
            dropoffChargeDetailsRequest = intent?.extras?.get("dropoffAddressCharge") as ChargeDetails
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            // (LaunchedEffect)->> avoid multiple times observer call, while recomposition
            LaunchedEffect(passengerDetailsViewModel.showValidationMessage) {
                passengerDetailsViewModel.showValidationMessage.observe(this@PassengerPaymentActivity) {
                    toast(it)
                }
            }

            if (passengerDetailsViewModel.selectedBookingType?.asString(resources) == getString(R.string.online_agent) ||
                passengerDetailsViewModel.selectedBookingType?.asString(resources) == getString(R.string.offline_agent))
            {
...
//                  toast("Pay via QR")
                    if (it.agentRechargeQrResp?.isNotEmpty()==true) {
                        try {
                            ...
                            passengerDetailsViewModel.bookTicketTotalFare = it.total_fare.toString()
                ...
                when (it.code) {
                    200 -> {
                        firebaseLogEvent(
                            this,
                            BOOKING_SUCCESS,
                            passengerDetailsViewModel.loginModelPref?.userName,
                            passengerDetailsViewModel.loginModelPref?.travels_name,
                            passengerDetailsViewModel.loginModelPref?.role,
                            BOOKING_SUCCESS,
                            "Booking success"
                        )
                        if (it.result.payment_initiatives == "RedelcomPay") {
                    ...
                    200 -> {
                        if (it.fare_break_up_hash != null) {
                            ...
                            for (i in 0..it.fare_break_up_hash.size.minus(1)) {
                                if ((it.fare_break_up_hash[i].value?.toString()?.toDoubleOrNull()
                                        ?: 0.0) > 0.0
                                )
        ...
        blockViewModel.userList.observe(this) { it ->
            ...
            if (it != null) {
                if (it.active_users != null && it.active_users.isNotEmpty() ) {
                    ...
                    if (passengerDetailsViewModel.selectedBranchId == 0 && passengerDetailsViewModel.selectedBookingTypeId!=7) {
            ...
            BoxWithConstraints {
                Box(
                    modifier = Modifier
                        ...
                        .padding(WindowInsets.systemBars  // includes status + nav bar
...
//                                        shape = RoundedCornerShape(8.dp)
                    ...
                    Column {
                        LazyColumn(
                            ...
                            item { TravelInfoCard(passengerDetailsViewModel) }

                            passengerDetailsViewModel.apply {
                                if (rapidBookingType != RAPID_TYPE_HIDE) {
                                    item {
                                        RapidBookingCard(passengerDetailsViewModel = passengerDetailsViewModel,
                                            onRapidBookingCheck = {
                                                rapidBookingSkip = it
                                                if (it) {
                                                    rapidBookingType = 0
                                                ...
                                                setPaymentOptionsVisibility(this@PassengerPaymentActivity,passengerDetailsViewModel)
                                            }
                                        )
                                    }
                                }
...
//                                    passengerDetailsViewModel.showShimmer = true
                                    ...
                                    coroutineScope.launch {
                                        listState.animateScrollToItem(passengerDetailsViewModel.animateScrollToItemPosition)
                                    }
                                }
...
//                                                }
                                ...
                                if (rapidBookingType != 0) {
                                    item { ContactDetails(passengerDetailsViewModel) }
                                        ...
                                        InsuranceCard(
                                            ...
                                            onInsuranceChecked = {
                                                passengerDetailsViewModel.isInsuranceChecked.value =
                                                    it
                                                handlePartialPaymentPrivilege(this@PassengerPaymentActivity,
                                                    passengerDetailsViewModel,
                                                    onPartialPaymentChange = {
                                                        paymentRadioListener(
                                                            passengerDetailsViewModel
                                ...
                                itemsIndexed(passengerDataList) { index, item ->
                                    ItemPassengerDetails(this@PassengerPaymentActivity,
                                        passengerDetailsViewModel,
                                        item,
                                        index,
                                        onMealChecked = {
                                ...
                                if (rapidBookingType != 0) {
                                    ...
                                    if (isFullPartialCardVisible && PreferenceUtils.getPreference(PREF_PICKUP_DROPOFF_CHARGES_ENABLED, false) == false) item {
                                        FullPartialPaymentCard(this@PassengerPaymentActivity,
                                            passengerDetailsViewModel,
                        ...
                        Column(
                            modifier = if (passengerDetailsViewModel.isFareBreakupBottomSheetVisible) Modifier.defaultMinSize(minHeight =
                                50.dp
                            ) else if (passengerDetailsViewModel.getAvailableBalance.isNotEmpty()) Modifier.defaultMinSize(minHeight =
                                2.dp
                            ) else Modifier.defaultMinSize(minHeight =26.dp)
                        ) {
...
//                                                    )
    ...
    private fun openCalender() {
        ...
        SlyCalendarDialog()
            ...
            .setCallback(this@PassengerPaymentActivity)
    ...
    private fun callUserListApi() {
        ...
        if(passengerDetailsViewModel.selectedBookingTypeId == 7) {
            userId = 1
        }
            ...
            CardComponent(
                ...
                Row(
                    ...
                    Row(
                        ...
                        TextBoldLarge(
                            text = if (passengerDetailsViewModel.partialAmount == 0.0 || !passengerDetailsViewModel.isPartialPayment || passengerDetailsViewModel.isInsuranceChecked.value) "${passengerDetailsViewModel.amountCurrency} ${
                                passengerDetailsViewModel.totalFareString.toDouble()
                                    .convert(passengerDetailsViewModel.currencyFormat)
                            }" else "${passengerDetailsViewModel.amountCurrency} ${
                        ...
                        IconButton(onClick = { onClick(FARE_DETAILS) },
                            modifier = Modifier.semantics { role = Role.Button }) {
                            Icon(
                                modifier = Modifier.semantics { role = Role.Button },
                                imageVector = if (passengerDetailsViewModel.isFareBreakupBottomSheetVisible) ImageVector.vectorResource(
                    ...
                    if (passengerDetailsViewModel.rapidBookingSkip) {
                        CreateButton(
                            enabled = true,
                            modifier = Modifier
                                .semantics { role = Role.Button }
...
//                                .navigationBarsPadding()
                                ...
                                .weight(1f),
                            text = if (passengerDetailsViewModel.phoneBlock) stringResource(id = R.string.do_phone_booking) else stringResource(
                                id = R.string.confirm_and_book
                            ),
                            onClick = {
                                passengerDetailsViewModel.isCancelledClicked = false
                                onClick(BOOK_TICKET)
                        ...
                        val isEnable = if (passengerDetailsViewModel.phoneBlock || passengerDetailsViewModel.isCashEnabled) {
                            passengerDetailsViewModel.isAllMandatoryFieldsFilled.size == passengerDetailsViewModel.passengerDataList.size
                        } else {
                            passengerDetailsViewModel.isAllMandatoryFieldsFilled.size == passengerDetailsViewModel.passengerDataList.size
                                    && passengerDetailsViewModel.selectedPaymentOptionId != 1
...
//                                .navigationBarsPadding()
                                ...
                                .align(CenterVertically)
                                .background(
                                    if (passengerDetailsViewModel.isAllMandatoryFieldsFilled.size != passengerDetailsViewModel.passengerDataList.size) {
            ...
            if (privilegeResponse != null) {
                passengerDetailsViewModel.apply {
                    ...
                    setBoardingDroppingData(
                        passengerDetailsViewModel.roleType,
                    ...
                    setRemarksCardVisibility(hide = getString(R.string.hide),mandatory = getString(R.string.mandatory))
                    ...
                    if (Build.MANUFACTURER.contains(
                            "PAX",
                            true
                        ) && privilegeResponse.isEzetapEnabledInTsApp == true && privilegeResponse.ezetapUserName != "" && privilegeResponse.ezetapApiKey != ""
                    ) {
...
//                                }
                                ...
                                passengerDetailsViewModel.isSeatWiseDiscountEdit = false

                                if (!passengerDetailsViewModel.isSeatWiseDiscountEdit) {
                                    passengerDetailsViewModel.isDiscountAmountChanged = true
                                }
                                passengerDetailsViewModel.animateScrollToItemPosition = 2
                            }
...
//                                passengerDetailsViewModel.roleDiscountValue = selectedDiscountValue
                                ...
                                passengerDetailsViewModel.isSeatWiseDiscountEdit = false
                                passengerDetailsViewModel.animateScrollToItemPosition = 4

//                              toast("user- $selectedDiscountValue")
                            }
                        }
...
//                groupRadio.visible()
                ...
                if (isNetworkAvailable()) {
                    if (passengerDetailsViewModel.shouldExtraSeatBooking && passengerDetailsViewModel.country.equals("india", true)) {
                        DialogUtils.showFullHeightPinInputBottomSheet(
                            activity = this@PassengerPaymentActivity,
    ...
    private fun bookTicketWithoutConfirm() {
        if (passengerDetailsViewModel.selectedSeatDetails.isNotEmpty()
            ...
            passengerDetailsViewModel.isExtraSeat = true
            Timber.d("booking_type- ExtraSeat")
            if (isNetworkAvailable()) {
                if (passengerDetailsViewModel.shouldExtraSeatBooking && passengerDetailsViewModel.country.equals("india", true)) {
                    DialogUtils.showFullHeightPinInputBottomSheet(
                        activity = this@PassengerPaymentActivity,
    ...
    private fun confirmPhoneBlockTicketApi() {
        if (isNetworkAvailable()) {
            val reqBody = com.bitla.ts.domain.pojo.photo_block_tickets.request.ReqBody(
                apiKey = passengerDetailsViewModel.loginModelPref?.api_key ?: "",
                paymentType = passengerDetailsViewModel.selectedPaymentOptionId,
                pnrNumber = passengerDetailsViewModel.pnrNumber,
                ticket = passengerDetailsViewModel.ticket.value,
                travelBranch = "",
                userId = passengerDetailsViewModel.userId,
                locale = passengerDetailsViewModel.locale
            )
    ...
    private fun bookSeatsWithExtraSeatApi() {
        ...
        val bookExtraSeatRequest = BookTicketWithExtraSeatRequest(
            ...
            is_bima_ticket = "${passengerDetailsViewModel.isBima}",
                        ...
                        passengerDataList.forEach {
                            ...
                            if (it.discountAmount.toString().isNotEmpty()) {
                                ...
                                val calculateDiscountValue =
                                    if (passengerDetailsViewModel.discountValue.toDouble() == 0.0
                                        || passengerDetailsViewModel.discountValue.toInt() == 0
                                    ) {
                                        it.discountAmount.toString().toDouble()
                                    } else {
                                        it.fare.toString()
                                            .toDouble() / 100.0f * it.discountAmount.toString()
                                            .toDouble()
                                    }
...
//        ----------additional Fare-------------
        ...
        reqBody.agentPaymentType = if (passengerDetailsViewModel.isAgentLogin && passengerDetailsViewModel.isAllowUpiForDirectPgBookingForAgents) {
        ...
        reqBody.pickupChargeDetails = pickupChargeDetailsRequest
        reqBody.dropoffChargeDetails = dropoffChargeDetailsRequest
...
//       val edcPackage="com.paytm.pos"
    ...
    private fun bookTicketApi(isUpi: Boolean, authPin: String?) {
        ...
        if (passengerDetailsViewModel.sourceId.contains("@")) {
            val temp = passengerDetailsViewModel.sourceId.split("@")
            if (passengerDetailsViewModel.isBima == false) {
                passengerDetailsViewModel.sourceId = temp[0]
            }
        }

        if (passengerDetailsViewModel.destinationId.contains("@")) {
            val temp = passengerDetailsViewModel.destinationId.split("@")
            if (passengerDetailsViewModel.isBima == false) {
                passengerDetailsViewModel.destinationId = temp[0]
            }
        }
        ...
        passengerDetailsViewModel.passengerDataList.forEach {
            ...
            seatDetail.title = it.sex ?: getString(R.string.empty)
                ...
                if (passengerDetailsViewModel.isAllowToApplyDiscountOnBookingPageWithPercentage) {
                    if (passengerDetailsViewModel.discountValue.toDouble() == 0.0
                        ...
                        if (it.discountAmount?.isNotEmpty() == true) {
                            ...
                            seatDetail.discountAmount =
                                it.discountAmount.toString().toDoubleOrNull()
                        }
                    }
            ...
            seatDetail.idCardType = it.idCardTypeId?.toInt()?:0
        ...
        if(passengerDetailsViewModel.isAgentLogin) {
            ...
            if (passengerDetailsViewModel.payableAmount > 0.0 && passengerDetailsViewModel.selectedPaymentOptionId != 1) {
                bookingDetails.payGayType = if (passengerDetailsViewModel.selectedSubPaymentOptionId == PaymentTypes.PHONEPE_V2) {
                    PayGayTypes.PHONEPE_V2
                } else {
                    PayGayTypes.EASEBUZZ
                }
            }
        } else {
//            bookingDetails.agentPaymentType =
...
//                }
        ...
        bookingDetails.pickupAddress = pickupChargeDetailsRequest.address
        bookingDetails.dropoffAddress = dropoffChargeDetailsRequest.address
...
//            bookingDetails.discountOnTotalAmount = discountOnTotalAmount.toString()
        ...
        if (passengerDetailsViewModel.selectedPaymentOptionId.toString() != "4"){
            if (!passengerDetailsViewModel.isAllowUpiForDirectPgBookingForAgents){
                bookingDetails.paymentType = passengerDetailsViewModel.selectedPaymentOptionId.toString()
            } else {
                if (passengerDetailsViewModel.selectedPaymentOptionId==1){
        ...
        //For Net Amt (less off commission)
        if (passengerDetailsViewModel.selectedPaymentOptionId.toString() == "16") {
            bookingDetails.paymentType = passengerDetailsViewModel.selectedPaymentOptionId.toString()
        }
        ...
        val reqBody = ReqBody()
        if (passengerDetailsViewModel.country.equals("india", true)) {
            reqBody.is_bima_ticket = "${passengerDetailsViewModel.isBima}"
        }
        ...
        if (passengerDetailsViewModel.isRapidBooking == "true") {
            passengerDetailsViewModel.apply {
                ...
                reqBodyWithRapidBooking.isOfflineBookedTic = true
        ...
        if(passengerDetailsViewModel.isAllowUpiForDirectPgBookingForAgents==true){
            bookingDetails.paymentType="1"
        }
        reqBody.deviceInfo =
    ...
    private fun bookExtraSeatApi(authPin: String) {
        ...
        if (passengerDetailsViewModel.privilegeResponseModel?.isEzetapEnabledInTsApp == true &&
            !passengerDetailsViewModel.phoneBlock) {
            if (passengerDetailsViewModel.selectedPaymentOptionId.toString() == "14") {
                bookingDetails.ezetapDeviceId = ezetapDeviceId
                bookingDetails.isEzetapPayment = true
            }
        ...
        reqBody.apiKey = passengerDetailsViewModel.loginModelPref?.api_key
        if (passengerDetailsViewModel.country.equals("india", true)) {
            reqBody.isBimaTicket = passengerDetailsViewModel.isBima
        }
        reqBody.departureDate = passengerDetailsViewModel.travelDate
        reqBody.operatorApiKey = operator_api_key
        reqBody.boardingAt = passengerDetailsViewModel.boardingId.toString()
        reqBody.bookingDetails = bookingDetails
        reqBody.contactDetail = contactDetail
        ...
        reqBody.deviceInfo = passengerDetailsViewModel.loginModelPref?.let { getDeviceInfo(loginModel = it) }
    ...
    companion object {
        val tag: String = PassengerPaymentActivity::class.java.simpleName
    }
