...
class BpDPBoardingListAdapter(
    private val context: Context,
    private var boardingDropping: MutableList<BoardingPointDetail>,
    private var onItemClickListener: OnItemClickListener,
) :
    RecyclerView.Adapter<BpDPBoardingListAdapter.ViewHolder>(), Filterable {
    private var lastSelectedPosition: Int = -1
    private var lastSelectedId: Int = -1
    private var boardingDroppingFiltered = boardingDropping

    companion object {
        var tag: String? = BpDPBoardingListAdapter::class.simpleName
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        val stageDetail: BoardingPointDetail = boardingDroppingFiltered[position]
        val bpSelected = PreferenceUtils.getString("preselectedBPData")
//        Timber.d("selectedDropping: ")
//        Timber.d("selectedDropping: ${PreferenceUtils.getObject<BoardingPointDetail>(
//            SELECTED_BOARDING_DETAIL)}")

        val selectedBoarding = PreferenceUtils.getObject<BoardingPointDetail>(
            SELECTED_BOARDING_DETAIL
        )
        Timber.d("selectedDropping: ${selectedBoarding}")
        /*val timeWithAddress =
...
//        Timber.d("selectedDropping: ${selectedBoarding}== ${stageDetail.id}")

        val isPickupDropoffChargesEnabled = PreferenceUtils.getPreference(PREF_PICKUP_DROPOFF_CHARGES_ENABLED, false) ?: false
        if (selectedBoarding?.id == "" && !isPickupDropoffChargesEnabled) {
            lastSelectedPosition = -1
            lastSelectedId = -1
        } else {
            if (selectedBoarding?.id == stageDetail.id) {
        ...
        if (boardingDropping[position].id == boardingPointId.toString() || boardingDropping[position].id == droppingPointId.toString()) {
        ...
        return object : Filter() {
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                boardingDroppingFiltered =
                    if (charString.isEmpty()) boardingDropping else {
                        val filteredList = mutableListOf<BoardingPointDetail>()
                        boardingDropping
                            .filter {
                                (it.name?.lowercase()
            ...
            override fun publishResults(constraint: CharSequence?, results: FilterResults?) {
                ...
                boardingDroppingFiltered = if (results?.values == null)
                    ...
                    results.values as MutableList<BoardingPointDetail>
                notifyDataSetChanged()
            }
        }
    }
