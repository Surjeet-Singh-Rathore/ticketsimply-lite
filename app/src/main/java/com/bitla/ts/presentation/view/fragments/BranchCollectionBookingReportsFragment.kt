...
import com.bitla.ts.databinding.LayoutBranchCollectionBookingFragmentBinding
...
import com.bitla.ts.presentation.adapter.BranchCollectionBookingAdapter
...
class BranchCollectionBookingReportsFragment(val reqBodyy: String?) : BaseFragment(),
    ...
    private lateinit var binding: LayoutBranchCollectionBookingFragmentBinding
    private var adapter: BranchCollectionBookingAdapter? = null
    ...
    override fun onCreateView(
        ...
        binding = LayoutBranchCollectionBookingFragmentBinding.inflate(inflater, container, false)
        binding.root
    ...
    private fun init() {
        ...
        binding.nextButtonBT.setOnClickListener {
            if(bookingData!!.result!!.noOfPagesBooked > lastPos + 1){
                pageAdapter!!.changeItemPosition(lastPos,lastPos+1)
    ...
    private fun getPref() {
        ...
        if (PreferenceUtils.getPreference(PREF_BRANCH_REPORT_DATA, "") != null) {
            ...
            if (!data.isNullOrEmpty()) {
                ...
                for (i in 0 until bookingData!!.result!!.noOfPagesBooked){
    ...
    private fun setAdpater(listData: AllReports) {

        if (listData.result != null) {
            adapter = BranchCollectionBookingAdapter(
                context = requireContext(),
                bookingDataList = listData.result.bookings,privilegeResponseModel
            )
