...
//    private var recyclerPassPassengerData: RecyclerPassPassengerData
) : RecyclerView.Adapter<PassengerDetailsAdapter.ViewHolder>() {
...
//    private var emailSeatMap = mutableMapOf<String, String>()
    ...
    override fun onBindViewHolder(holder: ViewHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        PassengerDetailsActivity.binding.dicountCodeDrop.setOnClickListener {
            if (couponName.isNullOrEmpty()) {
                context.toast(context.getString(R.string.no_coupon_available))
            } else {
                PassengerDetailsActivity.binding.dicountCodeDrop.showDropDown()
            }
        }
...
//      val layoutFemale = holder.layoutFemale
        ...
        holder.setItemFirstName(passengerItem.firstName)
        holder.setItemLastName(passengerItem.lastName)
                            ...
                            if (holder.etIdType.text.toString().equals("rut", true)) {
                                if (idNumber.toString().length == 10) {
                                    ...
                                    onItemPassData.onItemDataMore(
                                        holder.etIdNumber,
                                        passengerItem.seatNumber!!,
                                        holder.etIdNumber.text.toString(),
                                        idTypePosition.toString()
                                    )
                                }
                            }
                        } else {
...
//        holder.setItemIdNumber(passengerItem.idCardNumber.toString())
        ...
        holder.etAdditionalFare.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                try {
                    passengerDetailsResult[position].additionalFare = s.toString()
...
//                    it.idCardNumber=""
            ...
            onItemPassData.onItemData(
                holder.cross_icon_code,
                passengerItem.seatNumber.toString(),
                holder.etIdNumber.text.toString()
            )
        }
...
//            dialog.show()
        ...
        if (isInsuranceChecked) {
            holder.layoutName.gone()
            holder.layoutFirstLastName.visible()
            holder.layoutLastName.visible()
            holder.layoutage.visible()
            holder.genderContainer.visible()
        } else {
            if (isNameField.isNullOrEmpty())
                holder.layoutName.gone()
            if (isAgeField.isNullOrEmpty())
            ...
            if (isSexField.isNullOrEmpty())
                holder.genderContainer.gone()
            holder.layoutFirstLastName.gone()
            holder.layoutLastName.gone()
        }

        Timber.d("isAgeField==> $isAgeField isSexField $isSexField isNameField $isNameField")

        holder.etFirstName.onChange {
            passengerItem.firstName = it
        }

        holder.etLastName.onChange {
            passengerItem.lastName = it
        }
    ...
    inner class ViewHolder(binding: ItemPassengerDetailsBinding) :
        ...
        var layoutFirstLastName = binding.layoutFirstLastName
        var layoutFirstName = binding.layoutFirstName
        var layoutLastName = binding.layoutLastName
        ...
        fun setItemFirstName(firstName: CharSequence?) = etFirstName.setText(firstName)

        fun setItemLastName(lastName: CharSequence?) = etLastName.setText(lastName)
        ...
        fun setItemAdditionalFare(additionalFare: CharSequence?) =
            etAdditionalFare.setText(additionalFare)

        fun setItemAdditionalDiscount(additionalDiscount: CharSequence?) =
            etDiscount.setText(additionalDiscount)
    ...
    private fun copyPassengerDetails(holder: ViewHolder, isChecked: Boolean, idTypeValue: String?) {
        ...
        if (passengerDetailsResult[0].name.toString().isEmpty()
            ...
            if (isChecked) {
                ...
                firstName = holder.etFirstName.text.toString()
                lastName = holder.etLastName.text.toString()
                ...
                passengerDetailsResult.apply {
                    ...
                    for (i in 0 until passengerDetailsResult.size) {
                        ...
                        if (i >= 0) {
                            ...
                            this[i].firstName = firstName.toString()
                            this[i].lastName = lastName.toString()
...
//                            passengerDetailsResult[i].idCardType = idTypeValue?.toIntOrNull()
                            ...
                            if (passengerFare != null && passengerFare!!.isNotEmpty())
                                this[i].additionalFare = passengerFare.toString()
                ...
                holder.apply {
                    setItemName(name)
                    setItemFirstName(firstName)
                    setItemLastName(lastName)
                    setItemAge(age)
                        ...
                        if (i == 0) {
                            ...
                            if (passengerFare != null && passengerFare!!.isNotEmpty())
                                passengerDetailsResult[i].additionalFare =
                                    passengerFare.toString()

                            if (passengerDiscount != null && passengerDiscount!!.isNotEmpty()) {
                                passengerDetailsResult[i].discountAmount = passengerDiscount.toString()
                            }
...
//                    }
    ...
    private fun checkIsAdditionalFareAndIsDiscount(
        holder: ViewHolder,
        isAdditionalFare: Boolean,
        isDiscountAmount: Boolean
    ) {
        if (isAdditionalFare && !isDiscountAmount) {
            holder.apply {
                tvAdditionalNdDiscountHeading.text =
                    context.getString(R.string.additional_fare_discount)
                mainLayout4.visible()
    ...
    fun insuranceCheckbox(
        isInsuranceChecked: Boolean,
        privilegePassengerAge: String?,
        privilegePassengerSex: String?
    ) {
        this.isInsuranceChecked = isInsuranceChecked
        if (privilegePassengerAge == context.getString(R.string.hide))
            isAgeField = ""
        if (privilegePassengerSex == context.getString(R.string.hide))
            isSexField = ""
        notifyDataSetChanged()
    }
}