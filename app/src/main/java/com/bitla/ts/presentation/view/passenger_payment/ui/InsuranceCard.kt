...
fun InsuranceCard(
    ...
    onInsuranceChecked: (Boolean) -> Unit
) {
    CardComponent(
    ...
    )
    {
        Row(
            ...
            Checkbox(
                checked = passengerDetailsViewModel.isInsuranceChecked.value,
                onCheckedChange = {
                    if (passengerDetailsViewModel.insuranceMandatoryForBookings.value == true)
                        onInsuranceChecked(true)
                    else
                        onInsuranceChecked(it)
                }
            )

            TextBoldSmall(
                text = stringResource(id = R.string.insurance).uppercase(),
