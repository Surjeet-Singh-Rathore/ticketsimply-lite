...
class ModifyIndividualRouteFareAdapter(
    private val context: Context,
    private var multistationFareDetailsList: MutableList<MultistationFareDetails>,
    private val onItemClickListener: OnItemClickListener,
    private var onFareChangeParent: ((item: MultistationFareDetails) -> Unit)

) :
    RecyclerView.Adapter<ModifyIndividualRouteFareAdapter.ViewHolder>() {
    ...
    override fun getItemCount(): Int {
        return multistationFareDetailsList.size
    }

    @SuppressLint("SetTextI18n")
    override fun onBindViewHolder(holder: ViewHolder, @SuppressLint("RecyclerView") position: Int) {

        val fareDetailsResponse: MultistationFareDetails = multistationFareDetailsList[position]
        val fareList = fareDetailsResponse.fareDetails

//        ---------Header----------
        holder.titleTV.text = "${fareDetailsResponse.origin_name} - ${fareDetailsResponse.destination_name}"
        ...
        //        ----------Upper Layout---------
        val modifyUpperLabelSeatFareAdapter = ModifyUpperLabelSeatFareAdapter(
            context = context,
...
//        holder.rvNestedUpper.setRecycledViewPool(viewPoolUpper)
        ...
        //        ----------Down Layout---------
        val modifyDownSeatFareValueAdapter = ModifyDownSeatFareValueAdapter(
            context = context,
            fareDetailsList = fareList,
            position,
        ) { item, childPosition ->
