...
class GroupByBranchReportApiBookingFragment : Fragment() {
    private var _binding: FragmentGroupByBranchTabBinding? = null
    private val binding get() = _binding!!
    private lateinit var apiBookingList: ArrayList<GroupByBranchReportBranchData>
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.getString(ARG_APIBOOKING_DATA)?.let {
            val type = object : TypeToken<ArrayList<GroupByBranchReportBranchData>>() {}.type
            apiBookingList = Gson().fromJson(it, type)
            binding.apply {
                if (apiBookingList.isNullOrEmpty()) {
                    rvBranch.gone()
                    NoResult.visible()
                } else {
                    rvBranch.visible()
    ...
    private fun setupRecyclerView() {
        binding.rvBranch.apply {
            ...
            adapter = GroupByBranchReportBranchAdapter(requireContext(), apiBookingList, currency, currencyFormat)
        }
    }
    ...
    companion object {
        private const val ARG_APIBOOKING_DATA = "apibooking_data"

        fun newInstance(apiBookingList: ArrayList<GroupByBranchReportBranchData>): GroupByBranchReportApiBookingFragment {
            return GroupByBranchReportApiBookingFragment().apply {
                arguments = Bundle().apply {
                    putString(ARG_APIBOOKING_DATA, Gson().toJson(apiBookingList))
                }
            }
        }
    }
