...
//                onItemClickListener.onClickOfItem(selectedChartId.toString(), 0)
        ...
        fun dialogRedelcomSerialNumber(
            context: Context,
            onItemClickListener: OnItemClickListener
        ) {
            ...
            LayoutInflater.from(context)
            builder.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))  // for transparent background
            val binding: DialogRedelcomSerialBinding =
                DialogRedelcomSerialBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)
            ...
            binding.btnVerify.setOnClickListener {
                onItemClickListener.onButtonClick(binding, builder)
                builder.cancel()
            }
        ...
        fun dialogBranchWiseRevenue(
            context: Context,
            onItemClickListener: OnItemClickListener,
            branchWiseRevenueDetailsList: List<Detail?>,
            branchName:String
        ) {
            ...
            LayoutInflater.from(context)
            builder.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))  // for transparent background
            val binding: DashboardBranchWiseRevenuePopupBinding =
                DashboardBranchWiseRevenuePopupBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)
            binding.BranchWiseL.tvSeats.text = "Seats: "+context.getString(R.string.dash)
            binding.BranchWiseL.tvRevenue.text = "Revenue: " +  context.getString(R.string.dash)

            binding.BranchWiseL.summaryRV.adapter = DashboardBranchWiseDetailsAdapter(context, branchWiseRevenueDetailsList = branchWiseRevenueDetailsList)
            binding.cancelIV1.setOnClickListener{
                binding.cancelIV1.visibility = View.GONE
            }
            binding.tv2.text = branchName
        ...
        fun dialogServiceWiseOccupancy(
            context: Context,
            onItemClickListener: OnItemClickListener,
            serviceWiseOccupancyDetailsList: List<com.bitla.ts.domain.pojo.service_occupancy_details_popup.Detail?>,
            serviceId:String
        ) {
            val detailList : MutableList<Detail> = mutableListOf()
            serviceWiseOccupancyDetailsList.forEach{
                val item = Detail(
                    revenue = it?.occupancy.toString() + "%",
                    seats = it?.seats.toString(),
                    user = it?.bookingMode
                )
                detailList.add(item)
            ...
            val binding: DashboardBranchWiseRevenuePopupBinding =
                DashboardBranchWiseRevenuePopupBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)
            binding.tv1.text = "Service-Wise Occupancy"
            binding.BranchWiseL.tvSeats.text = context.getString(R.string.seats)
            binding.BranchWiseL.tvRevenue.text = context.getString(R.string.occupancy)
            binding.BranchWiseL.tvBookingTypeHeader.text = context.getString(R.string.booking_mode)
            binding.BranchWiseL.summaryRV.adapter = DashboardBranchWiseDetailsAdapter(context,  detailList)
            binding.cancelIV1.setOnClickListener{
               builder.cancel()
            }
            binding.tv2.text = serviceId
            binding.btnOkay.setOnClickListener {
                onItemClickListener.onButtonClick(binding, builder)
                builder.cancel()
            }
        ...
        fun dialogFilterByOccupancy(
            context: Context,
            isServiceWiseBooking: Boolean,
            singleButtonListener: DialogSingleButtonListener? = null,
            onItemPassData: OnItemPassData? = null,
            ...
            LayoutInflater.from(context)
            builder.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))  // for transparent background
            val binding: DialogFilterBySingleBinding =
                DialogFilterBySingleBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)

            if (!isServiceWiseBooking) {
                binding.rangeSlider.gone()
                binding.occupancyHeader.gone()
            }

            val selectedServicePref =
                PreferenceUtils.getPreference(PREF_SELECTED_SERVICE_FILTER, "All Services")
            val selectedServicePref2 = PreferenceUtils.getPreference(
                PREF_SELECTED_SERVICE_FILTER_PENDING_QUOTA, "All Services"
            )

            if (selectedServiceName.isNullOrEmpty() && selectedServicePref2.isNullOrEmpty()) {
                if (selectedServiceName.isNullOrEmpty()) {
                    binding.etselectService.setText(selectedServicePref2)
                } else {
                    binding.etselectService.setText(selectedServicePref)
                }
            } else {
                binding.etselectService.setText(selectedServiceName)
            }

            binding.etselectService.setOnClickListener {
                singleButtonListener?.onSingleButtonClick("selectCity")
                builder.cancel()
            }
            binding.selectOriginSubHeaderLabel.setEndIconOnClickListener {
                singleButtonListener?.onSingleButtonClick("selectCity")
            ...
            var rangeFromValue = PreferenceUtils.getString(PREF_SLIDER_FROM_VALUE).toString()
            var rangeToValue = PreferenceUtils.getString(PREF_SLIDER_TO_VALUE).toString()

            if (rangeFromValue.isEmpty() && rangeToValue.isEmpty()) {
                binding.rangeSlider.setValues(0f, 100f)
            } else {
                binding.rangeSlider.setValues(rangeFromValue.toFloat(), rangeToValue.toFloat())
            }

            binding.rangeSlider.thumbRadius = 40
...
//                format.currency = Currency.getInstance("USD")
            ...
            binding.rangeSlider.addOnSliderTouchListener(object :
                ...
                override fun onStopTrackingTouch(slider: RangeSlider) {
                    rangeFromValue = slider.values[0].roundToInt().toString()
                    rangeToValue = slider.values[1].roundToInt().toString()
                    // Responds to when slider's touch event is being stopped
                }
            })

            binding.btnApplyFilter.setOnClickListener {
                onItemPassData?.onItemData(binding.btnDark, rangeFromValue, rangeToValue)
                PreferenceUtils.putString(PREF_SLIDER_FROM_VALUE, rangeFromValue)
                PreferenceUtils.putString(PREF_SLIDER_TO_VALUE, rangeToValue)
                builder.cancel()
            }

            binding.btnDark.setOnClickListener {
//                PreferenceUtils.removeKey(PREF_SLIDER_FROM_VALUE)
//                PreferenceUtils.removeKey(PREF_SLIDER_TO_VALUE)
...
//                PreferenceUtils.removeKey(PREF_SELECTED_SERVICE_FILTER)
        ...
        fun dialogSericeWiseSlider(
            context: Context,
            rangeStart: String,
            rangeEnd: String,
            onApplyFilter: ((
                newStartRange: String?,
                newEndRange: String?
            ) -> Unit)
        ) {
            val builder = AlertDialog.Builder(context).create()
            ...
            var rangeFromValue = rangeStart
            var rangeToValue = rangeEnd
            if (rangeFromValue.isEmpty() && rangeToValue.isEmpty()) {
                binding.rangeSlider.setValues(0f, 200f)
            } else {
                binding.rangeSlider.setValues(rangeFromValue.toFloat(), rangeToValue.toFloat())
            }
...
//                format.currency = Currency.getInstance("USD")
            ...
            binding.btnApplyFilter.setOnClickListener {
                onApplyFilter.invoke(rangeFromValue, rangeToValue)
        ...
        fun dialogJourneyByDashboard(
            context: Context,
            journeyBy: String?,
            dialogFilterByListener: ((newFilter: String) -> Unit)
        ) {
            var journeyByValue = journeyBy
            val builder = AlertDialog.Builder(context).create()
            LayoutInflater.from(context)
            builder.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))  // for transparent background
            val binding: DialogFilterByDashboardBinding =
                DialogFilterByDashboardBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)

            when (journeyByValue) {
                "doi" -> {
                    binding.rbDOI.isChecked = true
                }

                "doj" -> {
                    binding.rbDOJ.isChecked = true
                }
            ...
            binding.rbDOI.setOnClickListener {
                journeyByValue = "doi"
            }
            binding.rbDOJ.setOnClickListener {
                journeyByValue = "doj"
            }
            ...
            binding.btnSort.setOnClickListener {
                dialogFilterByListener.invoke(journeyByValue ?: "doi")
                builder.cancel()
            }
        ...
        fun dialogSortBy(
            context: Context,
            sortBy: String,
            newLowToHighLabel: String,
            newHighToLowLabel: String,
            showOccupancyFilter: Boolean,
            showDateFilter: Boolean,
            showNetAmountFilter: Boolean,
            ...
            LayoutInflater.from(context)
            builder.window?.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))  // for transparent background
            val binding: DialogSortByBinding =
                DialogSortByBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)

            binding.lowerToHigher.text = newLowToHighLabel
            binding.higherToLower.text = newHighToLowLabel
            if (showOccupancyFilter) {
                binding.lowerToHigher.visible()
                binding.higherToLower.visible()
            } else {
                binding.lowerToHigher.gone()
                binding.higherToLower.gone()
            }

            if (showDateFilter) {
                binding.dateOldToNew.visible()
                binding.dateNewToOld.visible()
            } else {
                binding.dateOldToNew.gone()
                binding.dateNewToOld.gone()
            }

            if (showNetAmountFilter) {
                binding.lowerToHigherNetAmount.visible()
                binding.higherToLowerNetAmount.visible()
            } else {
                binding.lowerToHigherNetAmount.gone()
                binding.higherToLowerNetAmount.gone()
            }

            when (sortBy) {
                "ltoh" -> {
                    binding.lowerToHigher.isChecked = true
                }

                "htol" -> {
                    binding.higherToLower.isChecked = true
                }

                "ltoh_netamt" -> {
                    binding.lowerToHigherNetAmount.isChecked = true
                }

                "htol_netamt" -> {
                    binding.higherToLowerNetAmount.isChecked = true
                }

                "ltoh_datewise" -> {
                    binding.dateOldToNew.isChecked = true
                }

                "htol_datewise" -> {
                    binding.dateNewToOld.isChecked = true
                }
            ...
            binding.lowerToHigher.setOnClickListener {
                sortByValue = "ltoh"
            }
            binding.higherToLower.setOnClickListener {
                sortByValue = "htol"
            }
            binding.lowerToHigherNetAmount.setOnClickListener {
                sortByValue = "ltoh_netamt"
//                sortByValue = "ltoh"
            }
            binding.higherToLowerNetAmount.setOnClickListener {
                sortByValue = "htol_netamt"
//                sortByValue = "htol"
            }
            binding.dateOldToNew.setOnClickListener {
                sortByValue = "ltoh_datewise"
            }
            binding.dateNewToOld.setOnClickListener {
                sortByValue = "htol_datewise"
            }
            binding.btnCancel.setOnClickListener {
                builder.cancel()
            }
        ...
        fun dialogOccupancyCalendar(
            ...
            val binding: DialogOccupancyCalendarBinding =
                DialogOccupancyCalendarBinding.inflate(LayoutInflater.from(context))
            builder.setCancelable(false)

//            val filterList = OccupancyCalendarModel("30", "20%")
//            occupancyCalendarList.add(0, filterList)

//            layoutManager = GridLayoutManager(context.applicationContext, 7)
//            binding.rvOccupancyCalendar.layoutManager = layoutManager

//            val filterAdapter = OccupancyCalendarAdapter(
//                context.applicationContext,
//                occupancyCalendarList
//            )
//            binding.rvOccupancyCalendar.adapter = filterAdapter
                ...
                init {
                    view.setOnClickListener {
                        if (day.owner == DayOwner.THIS_MONTH) {
                            ...
                            val selectedDateNew = "${day.date.yearMonth}-${day.date.dayOfMonth}"
//                            singleButtonListener.onSingleButtonClick(selectedDateNew)
                        }
                    }
                }
            }
            ...
            binding.calendarView.dayBinder = object : DayBinder<DayViewContainer> {
                ...
                override fun bind(container: DayViewContainer, day: CalendarDay) {
                    ...
                    /*try {
                        tvOccupancy.text = "${occupancyCalendarList[day.day - 1].occupancy}%"
...
//                        context.toast(e.message)
                    ...
                    Timber.d("calDate- ${day.date}, $selectedDates")
...
//                                textView.setBackgroundResource(R.drawable.layout_rounded_shape_occupancy_red)
            ...
            binding.exOneMonthText.text = title
//
            binding.calendarView.monthScrollListener = { month ->
                val title = "${monthTitleFormatter.format(month.yearMonth)} ${month.yearMonth.year}"
                binding.exOneMonthText.text = title

//                if(month.yearMonth == endMonth) {
//                    binding.exFiveNextMonthImage.invisible()
//                } else {
//                    binding.exFiveNextMonthImage.visible()
//                }


                if (month.yearMonth == startMonth) {
                    binding.exFivePreviousMonthImage.invisible()
                } else {
                    binding.exFivePreviousMonthImage.visible()
                }
...
//                    updateAdapterForDate(null)
            ...
            binding.exFiveNextMonthImage.setOnClickListener {
                binding.calendarView.findFirstVisibleMonth()?.let {
                    ...
                    /*context.toast(it.yearMonth.next.toString())
                    PreferenceUtils.setPreference("calendarView_next",it.yearMonth.next.toString())
                    clickListener?.onClickOfItem(it.yearMonth.next.toString(),it.yearMonth.next.atEndOfMonth().dayOfMonth)*/
