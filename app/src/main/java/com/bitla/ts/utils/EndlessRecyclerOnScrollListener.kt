...
abstract class EndlessRecyclerOnScrollListener(
    ...
    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
        ...
        val isNotAtBeginning = lastVisibleItem.plus(1) == totalItemCount
//        val isTotalMoreThanVisible = totalItemCount >= threshHold

        val shouldPaginate =
            isNoError && isNotLoadingAndIsNotLastPage && isAtLastItem && isNotAtBeginning
                    && isScrolling
        Timber.d("onpagenation:$firstVisibleItem + $visibleItemCount = $totalItemCount == $lastVisibleItem")
        Timber.d("onpagenation:$isNoError && $isNotLoadingAndIsNotLastPage && $isAtLastItem && $isNotAtBeginning &&  && $isScrolling")
